...
.PHONY: help dev dev-build dev-down dev-logs dev-restart dev-clean frontend-shell backend-shell db-shell install-frontend install-backend seed seed-clean dump-fixtures load-fixtures
...
# Загрузить демо-данные (категории + продукты + юзер demo/demo1234)
seed:
  docker exec backend python manage.py seed_demo

# Загрузить демо-данные с предварительной очисткой
seed-clean:
  docker exec backend python manage.py seed_demo --clear

# Сохранить текущие данные в фикстуру
dump-fixtures:
  docker exec backend python manage.py dumpdata \
    api.category api.product \
    --natural-foreign --natural-primary \
    --indent 2 \
    -o api/fixtures/demo.json
  @echo "✅ Фикстура сохранена в backend/api/fixtures/demo.json"

# Загрузить данные из фикстуры
load-fixtures:
  docker exec backend python manage.py loaddata api/fixtures/demo.json
